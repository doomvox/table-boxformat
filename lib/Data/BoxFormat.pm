...
Data::BoxFormat - work with the tabular data format generated by database SELECTs
...
=head1 SYNOPSIS
   ...
   # Or just supply input file name to the method, rather than the object
   ...
   my $data = $dbx->read_dbox();  # aref of arefs
...
sub read_dbox {
 ...
 RULERSCAN:
  foreach my $i ( 1 .. 2 ) { # ruler lines are always near top
    ...
    if( $line =~ m{ $ruler_line_rule }x ) { ## TODO rename this pattern?
  ...
  foreach my $i ( $header_loc, $first_data .. $last_data ) {
    ...
    my $beg = 0;
    foreach my $pos ( @pos ) {
      printf STDERR "beg: %6d, pos: %6d, line: %s\n", $beg, $pos, $line;
      my $val =
